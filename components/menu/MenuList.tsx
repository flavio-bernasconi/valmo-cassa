...
import { MenuItem, MenuItemTypes } from "@/app/protected/page";
...
export const MenuList = ({
  ...
  return (
    <div className="flex flex-col gap-6 relative w-full">
      <div className="sticky top-0 z-10 py-3 bg-slate-700/95 rounded-lg backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b -mx-2 px-4 flex gap-3">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <Input
            placeholder="Cerca un prodotto..."
            className="pl-10 bg-white/10 border-white/20  placeholder:text-slate-400 focus-visible:ring-slate-400"
            value={searchQuery}
                ...
                {items.map((item) => (
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <span className="font-semibold text-xl leading-tight group-hover:text-primary transition-colors">
                        {item.name}
                      </span>
